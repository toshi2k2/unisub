...
def get_peft_model_state_dict(
    ...
    elif config.peft_type == PeftType.SUBSPACEADAPTER:
        to_return = {}
        components = {}
        loadings = {}
        for k in state_dict:
            if "subspaceadapter_" in k:
                if save_subspaceadapter_components:
                    components.update(
                        {k: state_dict[k] for k in state_dict if "components" in k}
                    )
                if save_subspaceadapter_loadings:
                    loadings.update(
                        {k: state_dict[k] for k in state_dict if "loadings" in k}
                    )
        to_return.update(components)
        to_return.update(loadings)
    else:
...
def set_peft_model_state_dict(
    ...
    if config.peft_type in (
        ...
        parameter_prefix = {
            ...
            PeftType.HRA: "hra_",
            PeftType.SUBSPACEADAPTER: "subspaceadapter_",
        }[config.peft_type]
        for k, v in state_dict.items():
