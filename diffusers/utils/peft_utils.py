...
def get_peft_kwargs(
    ...
    if num_components is not None:
        components = list(num_components.values())[0]
    ...
    if network_alpha_dict is not None and len(network_alpha_dict) > 0:
        if len(set(network_alpha_dict.values())) > 1:
            ...
            lora_alpha = collections.Counter(network_alpha_dict.values()).most_common()[
                0
            ][0]

            # for modules with alpha different from the most occuring alpha, add it to the `alpha_pattern`
            alpha_pattern = dict(
                filter(lambda x: x[1] != lora_alpha, network_alpha_dict.items())
            )
            if is_unet:
                alpha_pattern = {
    ...
    if use_subspaceadapter:
        target_modules = list(
            {name.split(".subspaceadapter")[0] for name in peft_state_dict.keys()}
        )
    else:
        target_modules = list(
    ...
    if use_subspaceadapter:
        subspaceadapter_config_kwargs = {
            "r": r,
            "num_components": components,
            "target_modules": target_modules,
        }
        return subspaceadapter_config_kwargs
    else:
        lora_config_kwargs = {
            "r": r,
...
def delete_adapter_layers(model, adapter_name):
    ...
    if getattr(model, "_hf_peft_config_loaded", False) and hasattr(
        model, "peft_config"
    ):
