...
class StableDiffusionLoraLoaderMixin(LoraBaseMixin):
    ...
    def load_lora_weights(
        ...
        **kwargs,
        ...
        is_correct_format = all(
            "lora" in key or "dora_scale" in key or "subspaceadapter" in key
    ...
    def lora_state_dict(
        ...
        **kwargs,
        ...
        if all(
            ...
            state_dict, network_alphas = _convert_non_diffusers_lora_to_diffusers(
                state_dict
            )
    ...
    def load_lora_into_text_encoder(
        ...
        from peft import LoraConfig, SubspaceAdapterConfig
        ...
        if any(cls.text_encoder_name in key for key in keys):
            ...
            if len(text_encoder_lora_state_dict) > 0:
                ...
                peft_config_kwargs = get_peft_kwargs(
                    rank,
                    network_alphas,
                    text_encoder_lora_state_dict,
                    is_unet=False,
                )
                if "use_dora" in peft_config_kwargs:
                    if peft_config_kwargs["use_dora"]:
                        ...
                        if is_peft_version("<", "0.9.0"):
                            peft_config_kwargs.pop("use_dora")
                lora_config = LoraConfig(**peft_config_kwargs)
        ...
        **kwargs,
        ...
        if all(
            (
                ...
                or k.startswith("lora_te1_")
                or k.startswith("lora_te2_")
                or k.startswith("subspaceadapter_te_")
                or k.startswith("subspaceadapter_unet_")
                or k.startswith("subspaceadapter_te1_")
                or k.startswith("subspaceadapter_te2_")
            )
            for k in state_dict.keys()
        ):
            # Map SDXL blocks correctly.
        ...
        if any(cls.text_encoder_name in key for key in keys):
            ...
            if len(text_encoder_lora_state_dict) > 0:
                ...
                if use_subspaceadapter == True:
                    ...
                    for name, _ in text_encoder_attn_modules(text_encoder):
                        for module in ("out_proj", "q_proj", "k_proj", "v_proj"):
                            rank_key = f"{name}.{module}.subspaceadapter_B.loadings"
                            component_key = f"{name}.{module}.subspaceadapter_B.components"
                            if rank_key not in text_encoder_lora_state_dict:
                                continue
                            rank[rank_key] = text_encoder_lora_state_dict[
                                rank_key
                            ].shape[1]
                            num_components[component_key] = (
                                text_encoder_lora_state_dict[component_key].shape[1]
                            )
                    ...
                    subspaceadapter_config = SubspaceAdapterConfig(**subspaceadapter_config_kwargs)
                    ...
                    text_encoder.load_adapter(
                        adapter_name=adapter_name,
                        adapter_state_dict=text_encoder_lora_state_dict,
                        peft_config=subspaceadapter_config,
                    )
...
class FluxLoraLoaderMixin(LoraBaseMixin):
    ...
    def load_lora_into_transformer(
        ...
        if len(state_dict.keys()) > 0:
            ...
            if use_subspaceadapter == True:
                first_key = next(iter(state_dict.keys()))
                if "subspaceadapter_A" not in first_key:
                    state_dict = convert_unet_state_dict_to_peft(state_dict)
            ...
            elif use_subspaceadapter == False:
                first_key = next(iter(state_dict.keys()))
                if "lora_A" not in first_key:
                    state_dict = convert_unet_state_dict_to_peft(state_dict)
            ...
            inject_adapter_in_model(
                adapter_config, transformer, adapter_name=adapter_name
            )
            incompatible_keys = set_peft_model_state_dict(
                transformer, state_dict, adapter_name
            )
...
class AmusedLoraLoaderMixin(StableDiffusionLoraLoaderMixin):
    ...
    def load_lora_into_transformer(
        ...
        if network_alphas is not None:
            alpha_keys = [
                k for k in network_alphas.keys() if k.startswith(cls.transformer_name)
            ]
            network_alphas = {
                k.replace(f"{cls.transformer_name}.", ""): v
                for k, v in network_alphas.items()
