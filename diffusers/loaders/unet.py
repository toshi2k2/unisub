...
class UNet2DConditionLoadersMixin:
    ...
    def load_attn_procs(
        ...
        **kwargs,
        ...
        use_subspaceadapter = kwargs.pop("use_subspaceadapter", None)
        allow_pickle = False
        ...
        is_subspaceadapter = all(("subspaceadapter" in k) for k in state_dict.keys())
        is_model_cpu_offload = False
        is_sequential_cpu_offload = False

        if is_custom_diffusion:
            attn_processors = self._process_custom_diffusion(state_dict=state_dict)
        elif is_lora or is_subspaceadapter:
        ...
        if is_custom_diffusion and _pipeline is not None:
            is_model_cpu_offload, is_sequential_cpu_offload = (
                self._optionally_disable_offloading(_pipeline=_pipeline)
            )
    ...
    def _process_lora(
        ...
        if network_alphas is not None:
            alpha_keys = [
                k for k in network_alphas.keys() if k.startswith(unet_identifier_key)
            ]
            network_alphas = {
                k.replace(f"{unet_identifier_key}.", ""): v
        ...
        state_dict_to_be_used = (
            unet_state_dict if len(unet_state_dict) > 0 else state_dict
        )
        ...
        if len(state_dict_to_be_used) > 0:
            ...
            state_dict = convert_unet_state_dict_to_peft(
                state_dict_to_be_used, use_subspaceadapter
            )
            if network_alphas is not None:
                # The alphas state dict have the same structure as Unet, thus we convert it to peft format using
                # `convert_unet_state_dict_to_peft` method.
                network_alphas = convert_unet_state_dict_to_peft(
                    network_alphas, use_subspaceadapter=False
                )
            if use_subspaceadapter == True:
                rank = {}
                for key, val in state_dict.items():
                    if "subspaceadapter_B.loadings" in key:
                        rank[key] = val.shape[1]
                num_components = {}
                for key, val in state_dict.items():
                    if "subspaceadapter_B.components" in key:
                        num_components[key] = val.shape[1]
                subspaceadapter_config_kwargs = get_peft_kwargs(
                    rank,
                    num_components,
                    network_alphas,
                    state_dict,
                ...
                adapter_config = SubspaceAdapterConfig(**subspaceadapter_config_kwargs)
            if use_subspaceadapter == False:
                rank = {}
                num_components = None
                for key, val in state_dict.items():
                    if "lora_B" in key:
                ...
                adapter_config = LoraConfig(**lora_config_kwargs)
            ...
            inject_adapter_in_model(adapter_config, self, adapter_name=adapter_name)
            incompatible_keys = set_peft_model_state_dict(
                self, state_dict, adapter_name
            )
    ...
    def save_attn_procs(
        ...
        **kwargs,
        ...
        if is_custom_diffusion:
            ...
            if not USE_PEFT_BACKEND:
                raise ValueError(
                    "PEFT backend is required for saving LoRAs using the `save_attn_procs()` method."
                )
    ...
    def _convert_ip_adapter_image_proj_to_diffusers(
        self, state_dict, low_cpu_mem_usage=False
    ):
        ...
        if "proj.weight" in state_dict:
            ...
            heads = (
                state_dict["perceiver_resampler.layers.0.0.to_q.weight"].shape[0] // 64
            )
            ...
            for key, value in state_dict.items():
                ...
                diffusers_name = diffusers_name.replace(
                    "0.1.1.weight", "0.ff.1.net.0.proj.weight"
                )
                diffusers_name = diffusers_name.replace(
                    "0.1.3.weight", "0.ff.1.net.2.weight"
                )
                diffusers_name = diffusers_name.replace("1.1.0.", "1.ff.0.")
                diffusers_name = diffusers_name.replace(
                    "1.1.1.weight", "1.ff.1.net.0.proj.weight"
                )
                diffusers_name = diffusers_name.replace(
                    "1.1.3.weight", "1.ff.1.net.2.weight"
                )
                diffusers_name = diffusers_name.replace("2.1.0.", "2.ff.0.")
                diffusers_name = diffusers_name.replace(
                    "2.1.1.weight", "2.ff.1.net.0.proj.weight"
                )
                diffusers_name = diffusers_name.replace(
                    "2.1.3.weight", "2.ff.1.net.2.weight"
                )
                diffusers_name = diffusers_name.replace("3.1.0.", "3.ff.0.")
                diffusers_name = diffusers_name.replace(
                    "3.1.1.weight", "3.ff.1.net.0.proj.weight"
                )
                diffusers_name = diffusers_name.replace(
                    "3.1.3.weight", "3.ff.1.net.2.weight"
                )
            ...
            if cross_attention_dim is None or "motion_modules" in name:
                ...
                for state_dict in state_dicts:
                    if "proj.weight" in state_dict["image_proj"]:
                        ...
                        # IP-Adapter Plus
                        num_image_text_embeds += [
                            state_dict["image_proj"]["latents"].shape[1]
                        ]
                ...
                for i, state_dict in enumerate(state_dicts):
                    value_dict.update(
                        {
                            f"to_k_ip.{i}.weight": state_dict["ip_adapter"][
                    ...
                    value_dict.update(
                        {
                            f"to_v_ip.{i}.weight": state_dict["ip_adapter"][
                ...
                if not low_cpu_mem_usage:
                    ...
                    load_model_dict_into_meta(
                        attn_procs[name], value_dict, device=device, dtype=dtype
                    )
        ...
        for key_id, name in enumerate(self.attn_processors.keys()):
            for i, state_dict in enumerate(state_dicts):
                if f"{key_id}.to_k_lora.down.weight" in state_dict["ip_adapter"]:
                    ...
                    lora_dicts[i].update(
                        {
                            f"unet.{name}.to_k_lora.down.weight": state_dict[
                                "ip_adapter"
                    ...
                    lora_dicts[i].update(
                        {
                            f"unet.{name}.to_q_lora.down.weight": state_dict[
                                "ip_adapter"
                    ...
                    lora_dicts[i].update(
                        {
                            f"unet.{name}.to_v_lora.down.weight": state_dict[
                                "ip_adapter"
                    ...
                    lora_dicts[i].update(
                        {
                            f"unet.{name}.to_k_lora.up.weight": state_dict[
                                "ip_adapter"
                    ...
                    lora_dicts[i].update(
                        {
                            f"unet.{name}.to_q_lora.up.weight": state_dict[
                                "ip_adapter"
                    ...
                    lora_dicts[i].update(
                        {
                            f"unet.{name}.to_v_lora.up.weight": state_dict[
                                "ip_adapter"
                    ...
                    lora_dicts[i].update(
                        {
                            f"unet.{name}.to_out_lora.up.weight": state_dict[
                                "ip_adapter"
